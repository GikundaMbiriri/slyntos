...
exports.tumaCallback = exports.test = void 0;
 ...
 */
exports.tumaCallback = (0, https_1.onRequest)({
    cors: true,
    ...
    try {
        ...
        if (!callbackData.checkout_request_id ||
            !callbackData.merchant_request_id) {
            logger.error("❌ Invalid callback data - missing required fields", callbackData);
            res.status(400).json({
                success: false,
        ...
        if (sessionQuery.empty) {
            logger.error("❌ No payment session found for checkout_request_id:", callbackData.checkout_request_id);
            res.status(404).json({
                success: false,
        ...
        if (callbackData.status === "completed" &&
            ...
            try {
                ...
                logger.info("✅ Payment session updated with completed status for polling");
                res.status(200).json({
                    success: true,
            ...
            await sessionDoc.ref.update({
                status: "failed",
                paymentFailed: true,
                failureReason: callbackData.failure_reason || callbackData.result_desc,
                failedAt: admin.firestore.FieldValue.serverTimestamp(),
            });
            logger.info("❌ Payment session updated with failed status for polling");
            res.status(200).json({
                success: false,
                message: callbackData.failure_reason ||
