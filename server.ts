  ...
  app.post(
    ...
    async (req: any, res) => {
      ...
      try {
        ...
        const amount = plan === "pro" ? 1000 : 0; // $10 in cents
        db.prepare(
          `
        ...
        switch (event.type) {
          ...
          case "customer.subscription.deleted":
            ...
            if (user) {
              ...
              const endDate = subscription.current_period_end * 1000; // Convert to JS timestamp
              ...
              WHERE id = ?
            `,
              ).run(isActive ? 1 : 0, subscription.status, endDate, user.id);
